//- Отдельный класс для отрисовки надписей со статик функциями (меню)

//- Класс для сбора:
//  1.Режима работы
//  2.Конфигурации режима

//- Классы, которые будут отрисовывать данные для режимов. Это будут наследуемые классы. Здесь будет определяться режим
//и какой то фабрикой создаваться умный указатель на класс, исходя из которого будет определяться поведение - полиморфизм.
//Методы, необходимые в этом классе:
//1.Отрисовка названия режима
//2.Отрисовка краткой информации
//3.Основная часть (у каждого своя, но метод один - для унификации отрисовки всех стратегий) - для мониторинга это, например, отрисовка графика
//4.Итог

//- Фабрика режимов

//Этот класс остается, просто он соберет все вместе и будет вызывать функции менеджера
//Исходя из полученной конфигурации фабрика вернет умный указатель - shared наверное. Затем этот указатель вызовет приветственный метод
//затем при каждом пришедшем снапшоте он будет отрисовывать что то, исходя из своей задачи
